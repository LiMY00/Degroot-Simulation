...
class SFNetwork:
    ...
    def __init__(self, num_Doc, num_NonDoc, num_Influencer, num_stub_influencer, num_titled_doc, prob, random_state) -> None:
    ...
    def calculate_edge_weights(self):
        ...
        for edge in self.graph.edges():
            ...
            if agent_type1 == "Doctor" or agent_type1 == "Notitle_doctor" or agent_type1 == "Influencer":
                if node1 == node2:
                    weight = 1
            ...
            # Assign weights based on agent types
                if node1 != node2:
                    if agent_type2 == "Non_Doc" or agent_type2 == "Notitle_doctor" or agent_type2 == "Influencer":
                        weight = 0.003/(0.183*num_DocNeighbors + 0.003*(num_NonDocNeighbors+num_stub_Influencer) + 0.906)
                    else:
                        weight = 0.183/(0.183*num_DocNeighbors + 0.003*(num_NonDocNeighbors+num_stub_Influencer) + 0.906)
                else:                         
                    weight = 0.906/(0.183*num_DocNeighbors + 0.003*(num_NonDocNeighbors+num_stub_Influencer) + 0.906)
