...
class Norm_Agent:
    def __init__(self, NonDoc_id,  agent_type):
        ...
        lower, upper = 0, 1
        mu, sigma = 0.37, 0.25
        # self.opinion = stats.truncnorm.rvs(lower, upper, loc=mu, scale=sigma)
    ...
    def __init__(self, num_Doc, num_NonDoc, prob, initial_opinion_ratio, random_state) -> None:
        ...
        for node in self.graph.nodes():
            ...
            if random.random() < num_Doc/(num_Doc+num_NonDoc):
                agent = Stub_Agent(node, "Stubborn", initial_opinion_ratio)
    ...
    def calculate_edge_weights(self):
        ...
        for edge in self.graph.edges():
            ...
            # Assign weights based on agent types
                if node1 != node2:
                    weight = 0.09/(0.09*(num_Stub+num_Norm) + 0.8)
                else:                         
                    weight = 0.8/(0.09*(num_Stub+num_Norm) + 0.8)
