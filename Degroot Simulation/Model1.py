...
class SFNetwork:
    ...
    def __init__(self, num_Doc, num_NonDoc, prob) -> None:
        self.graph = nx.barabasi_albert_graph(num_Doc+num_NonDoc, prob, seed=10)
        self.agents = {}
            ...
            # Assign weights based on agent types
                if node1 != node2:
                    if agent_type2 =="Non_Doc":
                        weight = 0.09*0.2/(0.19*num_DocNeighbors + 0.09*num_NonDocNeighbors)
                    else:
                        weight = 0.19*0.2/(0.19*num_DocNeighbors + 0.09*num_NonDocNeighbors)
                else:                         
                    weight = 0.8
